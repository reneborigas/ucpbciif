...
(function (global, factory) {
    ...
    /** @publicapi @module ng1 */ /** @hidden */ var ng_from_global = angular;
    ...
    (function () {
        ...
        function stateChangeStartHandler($transition$) {
            ...
            if (enabledEvents.$stateChangeSuccess) {
                var startEvent = $rootScope.$broadcast(
                    '$stateChangeStart',
                    $transition$.to(),
                    toParams,
                    $transition$.from(),
            ...
            if (enabledEvents.$stateChangeError) {
                $transition$.promise['catch'](function (error) {
                    if (
                        error &&
                        (error.type === 2 /* RejectType.SUPERSEDED */ || error.type === 3) /* RejectType.ABORTED */
                    )
                        return;
        ...
        function $StateEventsProvider($stateProvider) {
            ...
            var enabledStateEvents = allEvents
                .map(function (e) {
                    return [e, true];
                })
                .reduce(applyPairs, {});
