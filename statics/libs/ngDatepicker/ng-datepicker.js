(function () {
    ...
    pickerModule = angular.module('daterangepicker', ['angularMoment']);
    ...
    pickerModule.directive('dateRangePicker', [
        '$compile',
        '$timeout',
        '$parse',
        'dateRangePickerConfig',
        'moment',
        function ($compile, $timeout, $parse, dateRangePickerConfig, moment) {
            return {
                require: 'ngModel',
                restrict: 'A',
                scope: {
                ...
                link: function ($scope, element, attrs, modelCtrl) {
                    ...
                    modelCtrl.$parsers.push(function (viewValue) {
                        ...
                        objValue = opts.singleDatePicker
                            ? null
                            : {
                                  startDate: null,
                                  endDate: null,
                        ...
                        el.on('apply.daterangepicker', function (ev, picker) {
                            return $scope.$apply(function () {
                                if (opts.singleDatePicker) {
                                    ...
                                    !picker.startDate.isSame(picker.oldStartDate) ||
                                    !picker.endDate.isSame(picker.oldEndDate) ||
                                    !$scope.model ||
                                    !picker.startDate.isSame($scope.model.startDate) ||
                                    !picker.endDate.isSame($scope.model.endDate)
                                ) {
                                    $scope.model = {
                            ...
                            if (newClearable) {
                                return el.on('cancel.daterangepicker', function (ev, picker) {
                                    if (!picker.cancelingClick) {
                                        $scope.model = opts.singleDatePicker
                                            ? null
                                            : {
                                                  startDate: null,
                                                  endDate: null,
