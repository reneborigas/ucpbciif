define(function () {
    ...
    app.controller(
        ...
        function DocumentListController($http, $filter, $scope, toastr, NgTableParams, $state, $timeout, appFactory, $window, $stateParams, blockUI) {
            $scope.setUpDocumentTemplates = function () {
                $scope.templates = [
                    {
                        code: 'CLA',
                        templateUrl: '/statics/partials/pages/documents/list/creditline.html',
                    },
                    {
                        code: 'LA',
                        templateUrl: '/statics/partials/pages/documents/list/loanavailment.html',
                    },
                    {
                        code: 'LR',
                        templateUrl: '/statics/partials/pages/documents/list/loanrelease.html',
                    },
                ];
                    ...
                    function (response) {
                        ...
                        angular.forEach($scope.templates, function (template) {
                            angular.forEach($scope.subProcesses, function (subProcess) {
                                if (subProcess.code == template.code) {
                                    template.id = subProcess.id;
                                }
                            });
                        });
                        angular.forEach($scope.templates, function (template) {
                            if (template.id == $scope.subProcessId) {
            ...
            $scope.setUpDocumentTemplates();

            $scope.getDocumentTemplate = function () {
                if ($scope.currentTemplate) {
                    for (var i = 0; i < $scope.templates.length; i++) {
                        if ($scope.currentTemplate.id == $scope.templates[i].id) {
                ...
                $scope.tableDocuments = new NgTableParams(
                    ...
                    },
                    {
                        ...
                        getData: function (params) {
                            return $http.get('/api/documents/documentslist/', { params: { subProcessId: $scope.subProcessId } }).then(
                                function (response) {
    ...
    app.controller(
        ...
        function DocumentInfoController($http, $filter, $scope, toastr, NgTableParams, appFactory, $state, $timeout, blockUI, $q, $window) {
    ...
    app.controller(
        'DocumentLoanReleasePrintController',
        function DocumentLoanReleasePrintController($http, $filter, $scope, toastr, NgTableParams, $state, $timeout, appFactory, $window) {
    ...
    app.controller(
        'DocumentAmortizationSchedulePrintController',
