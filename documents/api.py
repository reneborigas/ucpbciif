...
class GetDocumentFileName(views.APIView):
    def post(self, request):
        ...
        if not document:
            ...
            return Response({"status": "Accepted", "fileName": code + "-" + "001"}, status=status.HTTP_202_ACCEPTED)
        ...
        print(itemNumber)
        return Response({"status": "Accepted", "fileName": code + "-" + itemNumber}, status=status.HTTP_202_ACCEPTED)
            ...
            Document.objects.prefetch_related(
                ...
                Prefetch(
                    ...
                    queryset=DocumentMovement.objects.annotate(
                        ...
                        stepId=F("step_id"),
                        committeeName=Concat(
                            F("committee__firstname"),
                            V(" "),
                            F("committee__middlename"),
                            V(" "),
                            F("committee__lastname"),
                        ),
...
class DocumentMovementViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        queryset = (
            DocumentMovement.objects.annotate(
                documentId=F("document_id"),
                outputName=F("output__name"),
                outputId=F("output_id"),
                stepId=F("step_id"),
                committeeName=Concat(
                    F("committee__firstname"), V(" "), F("committee__middlename"), V(" "), F("committee__lastname")
                ),
                statusName=F("status__name"),
            )
            .exclude(isDeleted=True)
