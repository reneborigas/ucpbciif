...
class UserViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        queryset = (
            CustomUser.objects.prefetch_related(Prefetch("appAccess", UserApps.objects.all()))
            .annotate(
...
class UserLogsViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = UserLogs.objects.annotate(
            userName=F("user__profile__name"), content_typeText=F("content_type__model")
        ).order_by("-id")
        ...
        if action_type is not None:
            queryset = queryset.filter(action_type__contains=action_type)
